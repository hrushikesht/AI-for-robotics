...
class robot:
    ...
    def move(self, motion):
		beta=  motion[1]*tan(motion[0])/length
        
		if beta>0.001:
			R = motion[1]/beta
			cx=myrobot.x - sin(myrobot.orientation)*R
			cy=myrobot.y + cos(myrobot.orientation)*R
			myrobot.x = cx + sin(myrobot.orientation + beta)*R
			myrobot.y = cy - cos(myrobot.orientation + beta)*R
			myrobot.orientation = myrobot.orientation + beta
			result = [myrobot.x,myrobot.y,myrobot.orientation]
		else:
			myrobot.x=myrobot.x+motion[1]*cos(myrobot.orientation)
			myrobot.y=myrobot.y+motion[1]*sin(myrobot.orientation)
			myrobot.orientation=myrobot.orientation
		result=myrobot
