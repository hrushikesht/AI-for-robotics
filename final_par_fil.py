...
class robot:
    ...
    def move(self,motions):
		beta =  motions[1]*tan(motions[0])/length
		if beta>tolerance_orientation:
			...
			self.x = self.x + random.gauss(self.x,self.distance_noise)
			self.y = self.y + random.gauss(self.y,self.distance_noise)
			self.orientation = self.orientation + beta + random.gauss(0.0,self.steering_noise)
			self.orientation = self.orientation % (2*pi)
			result = [self.x,self.y,self.orientation]
		else:
			self.x=self.x+motion[1]*cos(self.orientation)
			self.y=self.y+motion[1]*sin(self.orientation)
			self.x = self.x + random.gauss(self.x,self.distance_noise)
			self.y = self.y + random.gauss(self.y,self.distance_noise)
			self.orientation=self.orientation + random.gauss(0.0,self.steering_noise)
			self.orientation = self.orientation % (2*pi)
		result = self
		return result  
...
N=500
X = particle_filter(motions, measurements,N)
